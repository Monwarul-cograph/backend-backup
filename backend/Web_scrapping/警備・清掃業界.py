...
def save_to_csv(category_name, names, links,job_link):
    ...
    filename = f"C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/警備・清掃業界{category_name}.csv"
...
# category_name = "警備・清掃業界"
# category_code = "16L"
# scrape_company_info(category_name, category_code, num_pages)


company_csv_files = glob.glob(os.path.join(
    "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/警備・清掃業界", '*.csv'
))
print(f'csv_files ~ {company_csv_files}')


def process_csv_files(csv_files, save_path):
    """CSVファイルを処理して企業情報をスクレイプする関数"""
    for file_path in csv_files:
        try:
            target_df = pd.read_csv(file_path)
            ...
            # required_columns = ['企業名', '企業情報リンク先', '求人企業リンク']
            ...
            #     continue  # Skip this file if any column is missing
            
            offer_company_info_data = []

            if target_df.get('企業情報リンク先') is not None:
                for i, company_url in enumerate(tqdm(list(map(lambda text: text[:38] + 'Jobs' + text[38:], target_df.get("企業情報リンク先").to_list())), desc="Processing companies")):
                    

                    if not isinstance(target_df['求人企業リンク'][i], str):
                        continue  # Skip if '求人企業リンク' is not a string

                    company_name = target_df["企業名"][i]
                    offer_company_info = scrape_company_offer_info(company_name, company_url)
                    offer_company_info_data.extend(offer_company_info)

                    if (i + 1) % 1000 == 0:
                        save_temp_csv(offer_company_info_data, save_path, i + 1)
                        offer_company_info_data = []
                    
                    time.sleep(1.1)  # Be respectful to the server

            if offer_company_info_data:
                save_temp_csv(offer_company_info_data, save_path, i + 1)
                
        except Exception as e:
            print(f"Error processing file {file_path}: {e}")

def retrieve_offer_company_csv_files(directory):
...
if __name__ == "__main__":
    ...
    print('1111')
    # Paths for input CSV files and output directory
    input_csv_path = "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/警備・清掃業界"
    output_csv_path = "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/警備・清掃業界/offer"
    ...
    company_csv_files = glob.glob(os.path.join(input_csv_path, '*.csv'))
    print(f'CSV files to process: {company_csv_files}')
    print('4444')
    # Process the CSV files to scrape company data
    process_csv_files(company_csv_files, output_csv_path)
    print('5555')
    # Get list of CSV files with scraped data
    offer_company_csv_files = retrieve_offer_company_csv_files(output_csv_path)
    print(f'Scraped data CSV files: {offer_company_csv_files}')
    print('6666')
# def main():
#     """メイン関数"""
#     for category_name, category_code in DODA_CATEGORIES.items():  # Assuming DODA_CATEGORIES is defined elsewhere
#         print(f"カテゴリ: {category_name}")
#         total_companies = get_total_companies(category_code)  # Assuming this function is defined elsewhere
#         num_pages = math.ceil(total_companies / 50)
#         scrape_company_info(category_name, category_code, num_pages)  # Assuming this function is defined elsewhere

#     csv_files = glob.glob(os.path.join("c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/警備・清掃業界/", '*.csv'))
#     process_csv_files(csv_files, "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/警備・清掃業界/")
