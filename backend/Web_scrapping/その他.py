...
KEN = '|'.join(KEN)   # join all item of ken with |

###User agent of various browser
...
def get_total_companies(category_code):
    ##(docstring)..use for purpose of this function
    ...
    response = requests.get(url)   #Send HTTP get requests in this url 
    soup = BeautifulSoup(response.content, 'html.parser')  #HTML content parse by beautifulsoup
    result_span = soup.find('span', class_='search_result_number_unit_part')
    

    ## result span have any value or not??
    if result_span:
...
def save_to_csv(category_name, names, links,job_link):
    ...
    filename = f"C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/その他{category_name}.csv"
...
csv_files = glob.glob(os.path.join("C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/その他", '*.csv'))
...
for file_path in csv_files:
  ...
  for i, company_url in enumerate(tqdm(target_df.values.tolist(), desc="Processing companies")):
      ...
      if (i + 1) % 1000 == 0:
          save_temp_csv(company_info_data, "C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/その他", i + 1)
          company_info_data = []
      time.sleep(1.1)
