...
def save_to_csv(category_name, names, links,job_link):
    ...
    filename = f"C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/外食産業・飲食業界{category_name}.csv"
...
# category_name = "外食産業・飲食業界"
# category_code = "24L"
# scrape_company_info(category_name, category_code, num_pages)


csv_files = glob.glob(os.path.join("C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/外食産業・飲食業界", '*.csv'))
...
for file_path in csv_files:
  ...
  for i, company_url in enumerate(tqdm(target_df.values.tolist(), desc="Processing companies")):
      ...
      try:
        ...
        # TODO STUDY ===== START
        if target_df.get('求人企業リンク') is not None:
          if len(target_df.get('求人企業リンク')) >= int(i) :
            if isinstance(target_df.get('求人企業リンク')[i],str):
              url = scrape_doda_company_url(company_url[2])
          
          if isinstance(company_info, dict):
            if company_info.get('企業URL'):
              company_info.get['企業URL'] = url
        else:
          if isinstance(company_info, dict):
        ...
        company_info_data.append(company_info)
        if (i + 1) % 1000 == 0:
            save_temp_csv(company_info_data, "C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/外食産業・飲食業界", i + 1)
            company_info_data = []
        time.sleep(1.1)
