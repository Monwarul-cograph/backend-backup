...
def save_to_csv(category_name, names, links,job_link):
    ...
    filename = f"C:/Users/user/Desktop/cograph_project/venv{category_name}.csv"
...
def doda_company_split(file_names):
  """企業の詳細情報をスクレイピングする関数"""                                                                                                          #####tarinai
  for i,file in enumerate(file_names):
    df = pd.read_csv(file)
    df_offer = df[df['求人有無'].str.match(r'^\d{1,2}$')]
    df_no_offer = df[df['求人有無'].str.match(r'~^\d{1,2}$')]
    save_temp_csv(df_offer.values.tolist(), "C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/農林水産・鉱業/company_table/offer", i + 1)
    save_temp_csv(df_no_offer.values.tolist(), "C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/農林水産・鉱業/company_table/no_offer", i + 1)
...
def scrape_company_offer_info(company_name,base_url):
  ...
  for page in range(1, get_offer_page_number(base_url)+1):
      ...
      for card in cards:
          ...
          job_listings.append({'Company_name':company_name,"job_tittle":job_tittle,'job_description':job_description,"location": location})
      time.sleep(1)
  return job_listings
...
# # category_code に指定された業界（大項目）      "農林水産・鉱業": "21L",
# category_code = "21L"
...
csv_files = glob.glob(os.path.join("C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/農林水産・鉱業", '*.csv'))




company_csv_files= glob.glob(os.path.join(
    "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/農林水産・鉱業/company_table", '*.csv'
))
print(f'company_csv_files ~ {company_csv_files}')
...
for file_path in csv_files:
  ...
  i = 0
  for i, company_url in enumerate(tqdm(list(map(lambda text: text[:38] + 'Jobs' + text[38:], target_df["企業情報リンク先"].to_list())), desc="Processing companies")):
      ...
      if (i + 1) % 1000 == 0:
          save_temp_csv(offer_company_info_data, "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/農林水産・鉱業/company_table/offer", i + 1)
          offer_company_info_data = []
      time.sleep(1.1)
  ...
  def main():
    """メイン関数"""
    for category_name, category_code in DODA_CATEGORIES.items():
        print(f"カテゴリ: {category_name}")
        total_companies = get_total_companies(category_code)
        num_pages = math.ceil(total_companies / 50)
        scrape_company_info(category_name, category_code, num_pages)

    csv_files = glob.glob(os.path.join("/content/drive/MyDrive/データサイエンス仙台ラボ/500. ラボ共通/組織運営/TUG統括/営業BDO連携/企業情報収集/doda/性能測定/企業名_リンク参照先のみ/", '*.csv'))
    process_csv_files(csv_files, "/content/drive/MyDrive/データサイエンス仙台ラボ/500. ラボ共通/組織運営/TUG統括/営業BDO連携/企業情報収集/doda/性能測定/業界/temp/")
...
#     filename = f"/content/drive/MyDrive/公社・官公庁・学校・研究施設_1/temp_company_info_{category_name}.csv"
#     df.to_csv(filename, index=False, encoding='utf-8')
#     print(f"中間ファイル '{filename}' を保存しました。")
