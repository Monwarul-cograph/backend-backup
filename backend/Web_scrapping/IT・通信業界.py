...
def save_to_csv(category_name, names, links,job_link):
    ...
    filename = f"C:/Users/user/Desktop/cograph_project/venv/cographPJ/backendtask{category_name}.csv"
...
csv_files = glob.glob(os.path.join(
    "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/IT・通信業界", '*.csv'
))
        ...
        try:
            ...
            if (i + 1) % 1000 == 0:
                try:
                    save_temp_csv(
                        company_info_data, 
                        "c:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/IT・通信業界", 
                        i + 1
                    )
                    company_info_data = []
                except Exception as e:
...
# csv_files = glob.glob(os.path.join("C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/IT・通信業界", '*.csv'))
...
#       if (i + 1) % 1000 == 0:
#           save_temp_csv(company_info_data, "C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/IT・通信業界", i + 1)
#           company_info_data = []
#       time.sleep(1.1)

company_csv_files = glob.glob(os.path.join("/content/drive/MyDrive/公社・官公庁・学校・研究施設_1/thien", '*.csv'))
company_csv_files
for file_path in csv_files:
  target_df = pd.read_csv(file_path)
  print(file_path)
  ...
  for i, company_url in enumerate(tqdm(list(map(lambda text: text[:38] + 'Jobs' + text[38:], target_df["企業情報リンク先"].to_list())), desc="Processing companies")):
      ...
      if (i + 1) % 1000 == 0:
          save_temp_csv(offer_company_info_data, "/content/drive/MyDrive/公社・官公庁・学校・研究施設_1/offer", i + 1)
          offer_company_info_data = []
      time.sleep(1.1)

  if offer_company_info_data:
    # /content/drive/MyDrive/IT_Company/thien
      save_temp_csv(offer_company_info_data, "/content/drive/MyDrive/公社・官公庁・学校・研究施設_1/offer", i + 1)
offer_company_csv_files = glob.glob(os.path.join("/content/drive/MyDrive/公社・官公庁・学校・研究施設_1/thien/offer", '*.csv'))
offer_company_csv_files
scrape_all_contact_company_url(offer_company_csv_files)

df_temp_1 = pd.DataFrame([company_info_data[0]])
for num in range(1, len(company_info_data)):
    df_temp_2 = pd.DataFrame([company_info_data[num]])
    df_temp_1 = pd.concat([df_temp_1, df_temp_2], axis=0)
df_temp_1 = df_temp_1.reset_index(drop=True)
df_temp_1
