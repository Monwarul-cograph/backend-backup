...
def save_to_csv(category_name, names, links,job_link):
    ...
    filename = f"C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/メーカー（素材・化学・食品・化粧品・その他）業界.csv"
...
# category_name =  "メーカー（素材・化学・食品・化粧品・その他）業界"
# category_code = "03L"
# scrape_company_info(category_name, category_code, num_pages)



csv_files = glob.glob(os.path.join("C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/メーカー（素材・化学・食品・化粧品・その他）業界", '*.csv'))
...
for file_path in csv_files:
  ...
  for i, company_url in enumerate(tqdm(target_df.values.tolist(), desc="Processing companies")):
      ...
      if (i + 1) % 1000 == 0:
          save_temp_csv(company_info_data, "C:/Users/user/Desktop/cograph_project/cograph-database-pj/backend/data/メーカー（素材・化学・食品・化粧品・その他）業界", i + 1)
          company_info_data = []
      time.sleep(1.1)
